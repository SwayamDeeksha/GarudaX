...
export default function HomeDashboardScreen() {
  ...
  const totalInr = wallets.reduce(
    (sum, wallet) =>
      sum +
      (wallet.currency === "INR"
        ? wallet.availableBalance
        : wallet.availableBalance * (rates[`${wallet.currency}_INR`] || 83)),
    0
  );
    ...
    <Screen>
      ...
      <View className="flex-row items-center justify-between mb-5">
        ...
        <Pressable
          ...
          className="items-center justify-center w-12 h-12 rounded-xl bg-navy-800"
        >
          <Ionicons name="notifications" size={21} color="#F0C96A" />
          {unread ? (
            <View className="absolute w-3 h-3 bg-red-400 rounded-full right-2 top-2" />
          ) : null}
        </Pressable>
      </View>

      {/* 🔥 Compliance Card */}
      <View className="mb-4 rounded-2xl border border-white/15 bg-[#0F223A] p-4">
        <KycBanner status={user?.kycStatus || "Verified"} />
      </View>

      {/* 🔥 Total Balance Card */}
      <View className="mb-5 rounded-3xl border border-white/15 bg-[#0F223A] p-5">
        <Text className="text-sm text-slate-300">
          Total wallet balance equivalent
        </Text>
        ...
        <View className="flex-row gap-3 mt-4">
          ...
          <View className="flex-1">
            <AppButton
              ...
              onPress={() => router.push("/wallets/converter")}
            />
          </View>
        </View>
      </View>
      ...
      {wallets.slice(0, 3).map((wallet) => (
        <WalletCard
          key={wallet.currency}
          wallet={wallet}
          onPress={() => router.push(`/wallets/${wallet.currency}`)}
        />
      ))}
      ...
      <View className="mb-4 rounded-2xl border border-white/15 bg-[#0F223A] p-4">
        ...
        <Text className="mt-2 text-sm text-slate-300">
          USD/INR {rates.USD_INR || 83.12} • EUR/INR{" "}
          {rates.EUR_INR || 90.04} • AED/INR {rates.AED_INR || 22.64}
        </Text>
      </View>
